...
-- 'X.throwError'.
liftRethrow :: forall e r a . (Exc.Exception e, SetMember Lift (Lift IO) r, Member (Exc e) r)
  => (Exc.SomeException -> e) -> IO a -> Eff r a
...
runErrorRethrowIO
  :: forall e r a . (Exc.Exception e, SetMember Lift (Lift IO) r)
  => Eff (Exc e ': r) a -> Eff r a
runErrorRethrowIO =
  runError >=> either (Exc.throw . Exc.SomeException) return
